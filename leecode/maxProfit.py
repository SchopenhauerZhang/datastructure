...
#print(Solution()._maxProfit_k([3,3,5,0,0,3,1,4],2))
    ...
    def _maxProfit_63(self, prices: List[int]) -> int:
        """
        ...
        """
        if not prices or len(prices) <= 2:
            return prices[1] - prices[0] if len(prices) == 2 and prices[1] > prices[0] else 0
        flag = False
        for i in range(len(prices)):
            if i == 0 :
                continue
        ...
        for i in range(1,len(prices)):
            if i ==0:
                flag = max(flag,max(prices[i:]) - min(prices[:i]))
    ...
    def _maxProfit_63_eg(self, prices: List[int]) -> int:
